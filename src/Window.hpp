...
{
	HDC hdc=GetDC(hwnd);
	HFONT hf=CreateFontW(Fs,0,0,ltA,Bold,Italic,Underline,StrikeOut,0,0,0,0,0,Name);
	if(!hf)	MessageBoxW(NULL,L"Font creation failed!",L"Error",MB_OK|MB_ICONEXCLAMATION);
	ReleaseDC(hwnd,hdc);
	return hf;
}
...
*/
void PCDrawText(HWND hwnd,int X,int Y,HFONT Font,COLORREF col,LPCWSTR Text,int StrLen)
{
	HDC hdc=GetDC(hwnd);
	SelectObject(hdc,Font);
	SetTextColor(hdc,col);
	TextOutW(hdc,X,Y,Text,StrLen);
	ReleaseDC(hwnd,hdc);
}
